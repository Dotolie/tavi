...
static int M2T_PMTCallback (RMuint16 wPID, RMuint16 wType, void *context)
{
	MPEGDemux *pDemux = (MPEGDemux *)context;
	ASSERT (pDemux);
	return pDemux->M2T_PMTCallback (wPID, wType); 
}

static void M2T_PMSCallback (RMuint16 wPMSPID, RMuint16 wPID, RMuint16 wType, void *context)
{
	MPEGDemux *pDemux = (MPEGDemux *)context;
	ASSERT (pDemux);
	pDemux->M2T_PMSCallback (wPMSPID, wPID, wType); 
}

static RMuint16 M2T_PATCallback (RMuint16 wPID, RMuint16 wProgramNumber, void *context)
{
	MPEGDemux *pDemux = (MPEGDemux *)context;
	ASSERT (pDemux);
	return 	pDemux->M2T_PATCallback (wPID, wProgramNumber); 
}

static void M2TDemuxCallback (RMuint8 *pData, RMuint32 Length, RMuint16 PID, RMuint8 Flags, RMint64 offset, void *context)
{
	MPEGDemux *pDemux = (MPEGDemux *)context;
	ASSERT (pDemux);
	pDemux->M2TDemuxCallback (pData, Length, PID, Flags, offset);
}
...
MPG_DEMUX_ERROR MPEGDemux::Init ()
{
	...
	m_TSDemux.m_Callback = ::M2TDemuxCallback;
	m_TSDemux.m_PMTCallback = ::M2T_PMTCallback;
	m_TSDemux.m_PATCallback = ::M2T_PATCallback;
#ifdef PARSE_ALL_PROGRAM_MAP_TABLES
	m_TSDemux.m_PMSCallback = ::M2T_PMSCallback;
#endif
	m_PID0 = 0;
	m_PID1 = 0;
	m_TSDemux.m_Context = this;
	m_TSPes0.m_Callback = ::PESDemuxCallback;
	m_TSPes0.m_ac3dtsCallback = ::AC3DTSInfoCallback;
	m_TSPes0.m_lpcmCallback = ::LPCMInfoCallback;
	m_TSPes0.m_Context = this;
	m_TSPes0.m_IsDVDStream = 0;
	m_TSPes0.m_IsTSPESStream = 1;
	m_TSPes1.m_Callback = ::PESDemuxCallback;
	m_TSPes1.m_ac3dtsCallback = ::AC3DTSInfoCallback;
	m_TSPes1.m_lpcmCallback = ::LPCMInfoCallback;
	m_TSPes1.m_Context = this;
	m_TSPes1.m_IsDVDStream = 0;
	m_TSPes1.m_IsTSPESStream = 1;
...
MPG_DEMUX_ERROR MPEGDemux::DecodeFile (RMuint16 *filename)
{
	...
	if (IsTransportStream (filename))
	{
		...
		DEBUGMSG (1, ("TRANSPORT stream\n"));
		m_DemuxType = TRANSPORT_DEMUX;
	}
	else if (IsDVDStream (filename))
	{
...
int MPEGDemux::M2T_PMTCallback (RMuint16 wPID, RMuint16 wType)
{
	if (m_PID0 == 0)
	{
		if (m_CallbackTable.info (MSG_TS_PID0_TYPE, (void *)wType, m_CallbackTable.context))
		{
			m_PID0 = wPID;
			return 1;
		}
	...
	if ((m_PID1 == 0) && (wPID != m_PID0))
	{
		if (m_CallbackTable.info (MSG_TS_PID1_TYPE, (void *)wType, m_CallbackTable.context))
		{
			m_PID1 = wPID;
			return 1;
		}
...
void MPEGDemux::M2T_PMSCallback (RMuint16 wPMSPID, RMuint16 wPID, RMuint16 wType)
{
	ASSERT (0);
}

RMuint16 MPEGDemux::M2T_PATCallback (RMuint16 wPID, RMuint16 wProgramNumber)
{
	return wProgramNumber;
}

void MPEGDemux::M2TDemuxCallback (RMuint8 *pData, RMuint32 Length, RMuint16 PID, RMuint8 Flags, RMuint64 offset)
{
	if (PID == m_PID0)
	{
		// indicates that a new packet is starting
		// this allows for packets > 64k
		if (Flags)
			m_TSPes0.ResetState ();
		// send the result to the pes demux
		m_TSPes0.Demux (pData, Length);
	}
