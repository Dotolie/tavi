     ...
     gdbm_file_info *dbf;
{
  ...
  sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, (RMuint32)temp.av_adr, SEEK_SET );
  if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) {
  	_gdbm_fatal( dbf, "lseek_error" );
  }
  sdFatfsErr = fatfs_fread( (RMuint32)dbf->desc, (RMuint8*)&dbf->header->avail, 
  	(RMuint32)temp.av_size, (RMuint32*)&num_bytes );
     ...
     gdbm_file_info *dbf;
{
  ...
  {
  FATFS_ERROR sdFatfsErr;
  sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, (RMuint32)av_adr, SEEK_SET );
  if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) _gdbm_fatal( dbf, "lseek error" );
  sdFatfsErr = fatfs_fwrite( (RMuint32)dbf->desc, (RMuint32*)temp, 
  	(RMuint32)av_size, (RMuint32*)&num_bytes );
