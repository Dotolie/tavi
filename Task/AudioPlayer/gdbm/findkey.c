     ...
     int elem_loc;
{
  ...
  sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, 
  	(RMuint32)dbf->bucket->h_table[elem_loc].data_pointer, SEEK_SET );
  if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) _gdbm_fatal (dbf, "lseek error");
  sdFatfsErr = fatfs_fread( (RMuint32)dbf->desc, (RMuint8*)data_ca->dptr, 
  	(RMuint32)key_size+data_size, (RMuint32*)&num_bytes );
  if( num_bytes != key_size+data_size ) {
  	printf( "<1>failed to read: %d bytes, %d bytes\n", (RMuint32)key_size+data_size, num_bytes );
