     ...
     gdbm_file_info *dbf;
{
  ...
  sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, (RMuint32)0L, SEEK_SET );
  if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) _gdbm_fatal (dbf, "lseek error");
  fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)dbf->header, 
  	(RMuint32)dbf->header->block_size, (RMuint32*)&num_bytes );
  if( num_bytes != dbf->header->block_size ) _gdbm_fatal (dbf, "write error");
  #else
  ...
  if (dbf->directory_changed)
    {
      ...
      if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) _gdbm_fatal (dbf, "lseek error");
      sdFatfsErr = fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)dbf->dir, 
	  	(RMuint32)dbf->header->dir_size, (RMuint32*)&num_bytes );
      if( num_bytes != dbf->header->dir_size ) _gdbm_fatal (dbf, "write error");
      #else
