  ...
  if (dbf->cache_entry->ca_adr != bucket_adr)
	...
	_gdbm_write_bucket (dbf, &dbf->bucket_cache[dbf->last_read]);
      ...
      /* Read the bucket. */
       ...
       sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, bucket_adr, SEEK_SET );
	if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) {
		_gdbm_fatal( dbf, "lseek error" );
	}
	sdFatfsErr = fatfs_fread( (RMuint32)dbf->desc, (RMuint8*)dbf->bucket, 
					(RMuint32)dbf->header->bucket_size, (RMuint32*)&num_bytes );
       if( dbf->read_write != GDBM_READER ) {	
	      fatfs_fclose( (RMuint32)dbf->desc );
	...
	printf( "_gdbm_get_bucket: %d bytes, %d bytes, fs error: 0x%02lx\n", dbf->header->bucket_size, num_bytes, sdFatfsErr );
     ...
     cache_elem *ca_entry;
{
  ...
  printf( "_gdbm_write_bucket: pos(%d)\n", ca_entry->ca_adr );
  sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, (RMuint32)ca_entry->ca_adr, SEEK_SET );
  if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) {
  	_gdbm_fatal( dbf, "lseek error" );
  }
  sdFatfsErr = fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)ca_entry->ca_bucket, 
  		(RMuint32)dbf->header->bucket_size, (RMuint32*)&num_bytes );
  if( num_bytes != dbf->header->bucket_size )
