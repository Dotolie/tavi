     ...
     void (*fatal_func) ();
{
  ...
  if (flags == GDBM_READER)
    {
      #if FS_DRV_SIGMA
      sdFatfsErr = fatfs_fopen( (RMuint8*)dbf->name, _O_RDONLY, (RMuint32*)&dbf->desc );
      printf( "sdFatfsErr: 0x%x\n", sdFatfsErr );
      #else
    ...
    {
      #if FS_DRV_SIGMA
      sdFatfsErr = fatfs_fopen( (RMuint8*)dbf->name, _O_WRONLY, (RMuint32*)&dbf->desc );
      #else
    ...
    {
      #if FS_DRV_SIGMA
     sdFatfsErr =  fatfs_fopen( (RMuint8*)dbf->name, _O_WRONLY | _O_CREAT | _O_APPEND, (RMuint32*)&dbf->desc );
      #else
    ...
    {
      #if FS_DRV_SIGMA
      sdFatfsErr = fatfs_fopen( (RMuint8*)dbf->name, _O_WRONLY | _O_CREAT | _O_TRUNC | _O_APPEND, (RMuint32*)&dbf->desc );
      #else
      dbf->desc = open (dbf->name, O_RDWR|O_CREAT, mode);
      #endif /* FS_DRV_SIGMA */
      flags = GDBM_WRITER;
  ...
  if (dbf->desc < 0)
    {
      printf( "gdbm_open(flags:%d) : error: 0x%x\n", flags, sdFatfsErr );
      free (dbf->name);
      free (dbf);
  ...
  #if FS_DRV_SIGMA
  fatfs_fsize( (RMuint32)dbf->desc, (RMuint32*)&file_stat.st_size );
  #else
  ...
  if (flags == GDBM_READER)
	...
	{
	  #if FS_DRV_SIGMA
	  fatfs_fclose( (RMuint32)dbf->desc );
	  #else
	  close (dbf->desc);
	  #endif /* FS_DRV_SIGMA */
	  free (dbf->name);
  ...
  if (file_stat.st_size == 0)
	...
	= ( (dbf->header->block_size - sizeof (gdbm_file_header))
	 / sizeof (avail_elem)) + 1;
      ...
      #if FS_DRV_SIGMA
      sdFatfsErr = fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)dbf->header, 
      		dbf->header->block_size, (RMuint32*)&num_bytes );
	...
	{
         printf( "failed to write: %d bytes, %d bytes\n", dbf->header->block_size, num_bytes );
	  printf( "error: 0x%02lx\n", sdFatfsErr );
	  gdbm_close (dbf);
      ...
      #if FS_DRV_SIGMA
      sdFatfsErr = fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)dbf->dir, dbf->header->dir_size, (RMuint32*)&num_bytes );
      printf( "<2> write %d bytes\n", dbf->header->dir_size );	  
      #else
      num_bytes = write (dbf->desc, dbf->dir, dbf->header->dir_size);
      #endif
      if (num_bytes != dbf->header->dir_size)
	{
	  printf( "failed to write: %d bytes, %d bytes\n", dbf->header->dir_size, num_bytes );
	  printf( "error: 0x%02lx\n", sdFatfsErr );
	  gdbm_close (dbf);
	  gdbm_errno = GDBM_FILE_WRITE_ERROR;
	  return NULL;
      ...
      #if FS_DRV_SIGMA
      fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)dbf->bucket, 
      		dbf->header->bucket_size, (RMuint32*)&num_bytes );
      printf( "<3> write %d bytes\n", dbf->header->bucket_size );	  
      #else
      num_bytes = write (dbf->desc, dbf->bucket, dbf->header->bucket_size);
      #endif /* FS_DRV_SIGMA */
      if (num_bytes != dbf->header->bucket_size)
	{
	  printf( "failed to write: %d bytes, %d bytes\n", dbf->header->bucket_size, num_bytes );
	  printf( "error: 0x%02lx\n", sdFatfsErr );
	  gdbm_close (dbf);
	  gdbm_errno = GDBM_FILE_WRITE_ERROR;
	  return NULL;
  ...
  else
    {
	 ...
	 header and initialize the hash directory. */
      ...
      if( dbf->read_write != GDBM_READER ) {
	  	fatfs_fclose( (RMuint32)dbf->desc );
		fatfs_fopen( (RMuint8*)dbf->name, _O_RDONLY, (RMuint32*)&dbf->desc );
      	}
      fatfs_fread( (RMuint32)dbf->desc, (RMuint8*)&partial_header, sizeof(gdbm_file_header), (RMuint32*)&num_bytes );
      if( dbf->read_write != GDBM_READER ) {
	  	fatfs_fclose( (RMuint32)dbf->desc );
		fatfs_fopen( (RMuint8*)dbf->name, _O_WRONLY, (RMuint32*)&dbf->desc );
      	}
      #else
      ...
      if (dbf->header == NULL)
	{
      ...
      fatfs_fread( (RMuint32)dbf->desc, (RMuint8*)&dbf->header->avail.av_table[1], 
	  	dbf->header->block_size-sizeof (gdbm_file_header), (RMuint32*)&num_bytes );
      if( dbf->read_write != GDBM_READER ) {
	  	fatfs_fclose( (RMuint32)dbf->desc );
		fatfs_fopen( (RMuint8*)dbf->name, _O_WRONLY, (RMuint32*)&dbf->desc );
      	}
      ...
      if (dbf->dir == NULL)
	{
      ...
      {
      FATFS_ERROR sdFatfsErr;
      sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, (RMuint32)dbf->header->dir, SEEK_SET );
      fatfs_ftell( (RMuint32)dbf->desc, (RMuint32*)&file_pos );  	 
      if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) file_pos = -1;
      }
      #else
      ...
      if (file_pos != dbf->header->dir)
	{
      ...
      fatfs_fread( (RMuint32)dbf->desc, (RMuint8*)dbf->dir, (RMuint32)dbf->header->dir_size, (RMuint32*)&num_bytes );
      if( dbf->read_write != GDBM_READER ) {
	  	fatfs_fclose( (RMuint32)dbf->desc );
		fatfs_fopen( (RMuint8*)dbf->name, _O_WRONLY, (RMuint32*)&dbf->desc );
      	}
