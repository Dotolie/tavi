     ...
     int flags;
{
  ...
  RMuint32 size;
  #if 0
  fatfs_fsize( (RMuint32)dbf->desc, &size );
  if( (file_adr - size) > 0 ) {
  	RMuint32 len;
	RMuint8* pBuf;
	len = (key.dsize+content.dsize)+100;
  	pBuf = ( RMuint8* )malloc( len );
	memset( pBuf, 0, len );
	printf( "write %d bytes\n", len );
	fatfs_fseek( (RMuint32)dbf->desc, 0, SEEK_END );
  	fatfs_fwrite( (RMuint32)dbf->desc, pBuf, len, (RMuint32*)&num_bytes );
	free( pBuf );
  }
  #endif
  fatfs_ftell( (RMuint32)dbf->desc, (RMuint32*)&num_bytes );
  printf( "<b>file position: %d\n", num_bytes );
  sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, (RMuint32)file_adr, SEEK_SET );
  //sdFatfsErr = fatfs_fseek( (RMuint32)dbf->desc, 0, SEEK_END );
  printf( "gdbm_store: file_adr: %d, key_size: %d\n", file_adr, key.dsize );
  if( sdFatfsErr != FATFS_ERROR_NO_ERROR ) _gdbm_fatal (dbf, "lseek error");
  fatfs_ftell( (RMuint32)dbf->desc, (RMuint32*)&num_bytes );
  printf( "<1>file position: %d\n", num_bytes );
  sdFatfsErr = fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)key.dptr, (RMuint32)key.dsize, (RMuint32*)&num_bytes );
  if( num_bytes != key.dsize ) {
  	printf( "gdbm_store<1>: error: 0x%02lx\n", sdFatfsErr );
	printf( "%d bytes, %d bytes, addr: %d\n", content.dsize, num_bytes, file_adr );
  	_gdbm_fatal (dbf, "write error");
  }
  fatfs_ftell( (RMuint32)dbf->desc, (RMuint32*)&num_bytes );
  printf( "<2>file position: %d, content size: %d\n", num_bytes, content.dsize );
  sdFatfsErr = fatfs_fwrite( (RMuint32)dbf->desc, (RMuint8*)content.dptr, (RMuint32)content.dsize, (RMuint32*)&num_bytes );
  if( num_bytes != content.dsize ) {
  	printf( "gdbm_store<2>: error: 0x%02lx\n", sdFatfsErr );
	printf( "%d bytes, %d bytes\n", content.dsize, num_bytes );
  	_gdbm_fatal (dbf, "write error");
  }
  }
  #else
